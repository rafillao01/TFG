...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Consultar y actualizar los datos del usuario
    $sql = "UPDATE USUARIOS SET 
            Nombre = IF(? <> '', ?, Nombre), 
            Apellidos = IF(? <> '', ?, Apellidos), 
            Correo = IF(? <> '', ?, Correo), 
            Telefono = IF(? <> '', ?, Telefono), 
            Contrasena = IF(? <> '', ?, Contrasena) 
            WHERE NombreUsuario = ?";
    $statement = $conexion->prepare($sql);
    $statement->bind_param("sssssssssss", $nombre, $nombre, $apellidos, $apellidos, $correo, $correo, $telefono, $telefono, $contrasena, $contrasena, $nombreUsuario);
    ...
    if ($statement->execute()) {
        ...
        echo '<script>alert("Error al modificar usuario" '. $statement->error. '");</script>';
    }
...
$nombre = $apellidos = $nombreUsuario = $correo = $telefono = "";

if (isset($_SESSION['usuario'])) {
    $nombreUsuario = $_SESSION['usuario'];

    $sql = "SELECT Nombre, Apellidos, Correo, Telefono FROM USUARIOS WHERE NombreUsuario = ?";
    $statement = $conexion->prepare($sql);
    $statement->bind_param("s", $nombreUsuario);
    $statement->execute();
    $statement->bind_result($nombre, $apellidos, $correo, $telefono);
    $statement->fetch();
    $statement->close();
}
...
<body>
    ...
    <main>
        <div class="container">
            <h1 class="cabecera">Modificar Usuario</h1>
            <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" id="formulario" onsubmit="return validateForm()">
                ...
                <input type="text" id="usuario" name="nombreUsuario" value="<?php echo $nombreUsuario; ?>" readonly>
                
                <label for="correo" id="lcorreo">CORREO ELECTRONICO:</label>
                ...
                <input type="submit" value="MODIFICAR" id="botRegis">
            </form>
        </div>
    </main>
    <footer>
